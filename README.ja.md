...
- **マスターパスフレーズ** - PBKDF2（600k iterations）または Argon2id（64 MB）+ HKDF 鍵導出、Secret Key 付き
...
- **セッション管理** - アクティブセッション一覧と個別/全体失効; チームメンバー削除時の自動無効化
...
- **パスキーサインイン** - Discoverable FIDO2 クレデンシャルによるパスワードレスサインイン; PRF 対応キーは Vault を自動アンロック
- **メール + セキュリティキーサインイン** - メールアドレス検索による Non-discoverable クレデンシャル対応（タイミングオラクル対策付き）
- **マジックリンクサインイン** - ロケール対応テンプレートによるメールベースのパスワードレス認証
...
- **エラー追跡** - Sentry 統合（再帰的な機密データ除去付き; 平文シークレットがクライアント外に出ない）
- **CI セキュリティスキャン** - CodeQL SAST、Trivy コンテナスキャン、暗号ドメイン台帳検証、npm audit ブロッキング
...
- **鍵導出** - パスフレーズ → PBKDF2（600k）または Argon2id（64 MB）→ ラッピング鍵 → ランダム 256 ビット秘密鍵をラップ
...
- **チーム Vault** - E2E 暗号化（ECDH-P256）+ メンバーごとの鍵配布、エントリ単位の ItemKey 階層
...
- **パスキーサインイン** - Discoverable / Non-discoverable WebAuthn クレデンシャルによるサインイン（ユーザー列挙対策付き）
- **マジックリンクサインイン** - 期限付きメールトークンによるロケール対応テンプレート
...
- [暗号設計ホワイトペーパー（English）](docs/security/cryptography-whitepaper.md) — 鍵階層と暗号設計の全体像
- [脅威モデル STRIDE（English）](docs/security/threat-model.md) — 体系的な脅威分析

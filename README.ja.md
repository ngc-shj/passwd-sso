...
- **複数エントリタイプ** - パスワード、セキュアノート、クレジットカード、ID/個人情報、パスキー、銀行口座、ソフトウェアライセンス
...
- **CLI ツール** - Node.js CLI（`passwd-sso`）、8 コマンド: login, unlock, status, list, get, generate, totp, export; OS キーチェーン連携、XDG 準拠の設定管理
...
cli/
├── src/commands/             # CLI コマンド（login, unlock, list, get, generate, totp, export, status）
└── src/lib/                  # キーチェーン・設定・API クライアントヘルパー
```

...
- **インポート / エクスポート** — Bitwarden、1Password、Chrome CSV インポート; CSV/JSON エクスポート（AES-256-GCM 暗号化オプション付き）
...
- **アカウントロックアウト** — 段階的ロックアウト（5 回→15 分、10 回→1 時間、15 回→24 時間）
...
- **監査ログ & Webhook** — 個人/チームログ、フィルタ、CSV/JSONL ダウンロード、Webhook 配信
...
- **ブラウザ拡張** — Chrome/Edge MV3; 自動補完、インライン候補、AWS 3 フィールド、クレカ/住所、新規ログイン検出 & 保存
...
- **セッションセキュリティ** — データベースセッション（JWT ではない）、8 時間タイムアウト、15 分無操作で自動ロック
...
<details>
<summary>CI / 負荷テスト / ライセンススクリプト</summary>

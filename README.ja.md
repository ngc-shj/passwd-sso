...
これまで緩く読まれていた 3 つの環境変数が、環境スキーマの検証対象になりました。いずれも **fail-closed** です — 解釈できない値のまま動き続けるのではなく、プロセスが起動を拒否します。方向としては正しい変更ですが、これまで黙って許容されていた値を設定しているデプロイは、アップグレード後に**起動しなくなります**。ロールアウト前に 3 つとも確認してください。

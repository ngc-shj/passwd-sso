...
- **ネットワークアクセス制限** — テナント単位の CIDR 許可リスト（最初に評価され、Tailscale に依存しないロックアウト復旧手段）; Tailscale 連携を有効にすると、テールネット名を設定した場合はさらに WhoIs で正確なテールネットまで検証し、未設定の場合は Tailscale CGNAT アドレス帯の接続元であれば許可
- **アウトバウンド SSRF 対策** — テナントやユーザーの入力が影響しうる送信先へのすべての通信で、プライベート/ループバック/リンクローカル/CGNAT/メタデータ宛先、および標準化された IPv6 トランジショントンネル（Teredo、6to4、IPv4互換アドレス、RFC 6666 廃棄専用範囲）をブロック。既知の NAT64 プレフィックス（`64:ff9b::/96`）は内包された IPv4 アドレスをデコードして再チェック。オペレーターが独自に割り当てた NAT64 プレフィックス（RFC 7050 によるディスカバリ）はアドレスだけからは判別できないため対象外であり、これに依存する構成では別途 egress ファイアウォールの併用を推奨
...
**症状**: IdP が送出するテナントクレームが変わった場合（Google Workspace のドメイン変更、SAML 属性の変更など）、既存のテナントメンバーはサインイン時に拒否され、`audit_logs` に `AUTH_LOGIN_FAILURE` として記録されます。原因は **5 つ**あり、この CLI で直せるのは最初の 2 つだけです:

  ...
  ja: {
    ...
    body: (adminName: string, resetUrl: string) =>
      `<p>管理者 <strong>${adminName}</strong> があなたの保管庫のリセットを開始しました。</p>
       ...
       <p><a href="${escapeHtml(resetUrl)}" rel="noreferrer" style="display:inline-block;padding:10px 20px;background:#dc2626;color:#ffffff;text-decoration:none;border-radius:6px;">保管庫リセットを確認する</a></p>
  ...
  en: {
    ...
    body: (adminName: string, resetUrl: string) =>
      `<p><strong>${adminName}</strong> (an organization admin) has initiated a reset of your vault.</p>
       ...
       <p><a href="${escapeHtml(resetUrl)}" rel="noreferrer" style="display:inline-block;padding:10px 20px;background:#dc2626;color:#ffffff;text-decoration:none;border-radius:6px;">Confirm Vault Reset</a></p>

  ...
  ja: {
    ...
    body: (count: number, watchtowerUrl: string) =>
      `<p>Watchtower の自動監視により、保管庫内の <strong>${count}</strong> 件のエントリに関連する新しいデータ漏洩が検出されました。</p>
       ...
       <p><a href="${escapeHtml(watchtowerUrl)}" rel="noreferrer" style="display:inline-block;padding:10px 20px;background:#18181b;color:#ffffff;text-decoration:none;border-radius:6px;">Watchtower を確認する</a></p>`,
  ...
  en: {
    ...
    body: (count: number, watchtowerUrl: string) =>
      `<p>Watchtower auto-monitoring has detected new data breaches affecting <strong>${count}</strong> entry(ies) in your vault.</p>
       ...
       <p><a href="${escapeHtml(watchtowerUrl)}" rel="noreferrer" style="display:inline-block;padding:10px 20px;background:#18181b;color:#ffffff;text-decoration:none;border-radius:6px;">View Watchtower</a></p>`,
    text: (count: number, watchtowerUrl: string) =>

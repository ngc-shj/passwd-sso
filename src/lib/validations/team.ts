...
export const createTeamE2EPasswordSchema = z.object({
  ...
  aadVersion: aadVersionSchema,
  teamKeyVersion: z.number().int().min(1).max(TEAM_KEY_VERSION_MAX),
  itemKeyVersion: z.number().int().min(0).max(TEAM_KEY_VERSION_MAX).default(0),
...
export const updateTeamE2EPasswordSchema = z.object({
  ...
  aadVersion: aadVersionSchema.optional(),
  teamKeyVersion: z.number().int().min(1).max(TEAM_KEY_VERSION_MAX).optional(),
...
export const upsertTeamPolicySchema = z.object({
  ...
  // Nullable = inherit tenant. Server additionally enforces ≤ tenant value in the PATCH handler.

 ...
 * Excludes group:webhook to prevent self-referential triggers
 * (e.g. WEBHOOK_DELIVERY_FAILED re-triggering itself).
 */
...
export function TeamWebhookCard({ teamId, locale }: Props) {
  const t = useTranslations("TeamWebhook");
  ...
  const handleDelete = async (webhookId: string) => {
    try {
      const res = await fetchApi(apiPath.teamWebhookById(teamId, webhookId), {
        method: "DELETE",
      });
      if (res.ok) {

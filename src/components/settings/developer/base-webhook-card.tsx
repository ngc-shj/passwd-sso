...
export interface WebhookCardConfig {
  listEndpoint: string;
  ...
  locale: string;
  /** Reserved for future use; listEndpoint already encodes dynamic identifiers. */
  fetchDeps?: unknown[];
}
...
export function BaseWebhookCard({ config }: Props) {
  const {
    listEndpoint,
    createEndpoint,
    eventGroups,
  ...
  // Store deleteEndpoint in a ref to avoid stale closures without re-creating fetchWebhooks
  const deleteEndpointRef = useRef(config.deleteEndpoint);
  deleteEndpointRef.current = config.deleteEndpoint;
  ...
  const fetchWebhooks = useCallback(async () => {
    try {
      const res = await fetchApi(listEndpoint);
      if (res.ok) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (res.status === 400) {
        ...
        const urlErrors = (
          body?.details as { properties?: { url?: { errors?: unknown[] } } } | undefined
  ...
  const groupLabel = (key: string) => {
    return tAudit(groupLabelMap[key] ?? key);
  };

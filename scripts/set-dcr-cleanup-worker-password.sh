...
SQL="ALTER ROLE passwd_dcr_cleanup_worker WITH PASSWORD '${escaped_password}';"
...
if [[ "$DRY_RUN" == "1" ]]; then
  ...
  echo "[DRY_RUN] would invoke: psql \"${MIGRATION_DATABASE_URL}\" -f - <<< \"ALTER ROLE passwd_dcr_cleanup_worker WITH PASSWORD '<REDACTED>';\"" >&2

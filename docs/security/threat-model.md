...
| D5a: Bearer-token caller bypasses tenant IP restriction | `/api/v1/*`, `/api/extension/*`, `/api/mcp`, `/api/mcp/token`, `/api/mobile/*`, SCIM | `enforceAccessRestriction` / `checkAccessRestrictionWithAudit` enforce tenant `allowedCidrs` on Bearer-authenticated routes (incl. the MCP gateway POST + SSE, the MCP OAuth token endpoint's auth-code exchange + refresh rotation — gated on the resolved tenantId BEFORE minting/rotating so a stolen code/refresh token cannot be redeemed off-network — and the iOS token lifecycle) as well as session-cookie routes | Restriction only applies when `allowedCidrs` is configured; unconfigured tenants have no IP restriction on any route |

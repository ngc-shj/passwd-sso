...
6. **CSP `'wasm-unsafe-eval'` allows WebAssembly compilation in strict mode**: The production CSP retains `'wasm-unsafe-eval'` because `argon2-browser` (the WASM Argon2id implementation used by the vault KDF in `src/lib/crypto/crypto-client.ts`) cannot run without it. An attacker who successfully lands an XSS that survives `'strict-dynamic'` could then compile a WebAssembly module, partially bypassing the `'unsafe-eval'` defense. Mitigations: (a) `'strict-dynamic'` still gates the initial script execution, so an XSS must clear that bar first; (b) `'unsafe-eval'` (JS eval/Function) is NOT permitted in strict mode; (c) `worker-src 'self'` constrains WASM-in-Worker loads to same-origin only. Removing the directive would break vault setup / unlock entirely. Eliminating this risk requires migrating off argon2-browser to a non-WASM Argon2id implementation (or accepting the memory-hardness loss of native WebCrypto PBKDF2) — tracked as audit finding M2 (2026-05).

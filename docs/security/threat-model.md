...
| D5a: Bearer-token caller bypasses tenant IP restriction | `/api/v1/*`, `/api/extension/*`, `/api/mcp`, `/api/mcp/token`, `/api/mobile/*`, SCIM | `enforceAccessRestriction` / `checkAccessRestrictionWithAudit` enforce tenant `allowedCidrs` on Bearer-authenticated routes as well as session-cookie routes. Every credential-ISSUING Bearer endpoint gates on the resolved tenantId BEFORE the mint/rotation so a stolen credential cannot be redeemed off-network: the MCP gateway POST + SSE, the MCP OAuth token endpoint (auth-code exchange + refresh rotation), the extension bridge-code→token exchange, the iOS AutoFill token mint, and the iOS token lifecycle. Exception: OAuth token **revocation** (`/api/mcp/revoke`, RFC 7009) is intentionally NOT IP-gated — revocation must succeed from any network so a compromised token can always be killed | Restriction only applies when `allowedCidrs` is configured; unconfigured tenants have no IP restriction on any route |
| D6: DCR endpoint abuse (mass registration) | /api/mcp/register | IP rate limit (20/hr, IPv6 /64), global cap (1000 unclaimed, a table-bloat backstop) + 15min auto-expiry (lazy sweep at registration + worker) so the pool drains fast and can only be held full by a sustained attack. Open DCR (RFC 7591) cannot fully prevent cross-tenant exhaustion; these raise the cost (~10x, one-shot→sustained ~200 /64s) but do not eliminate it for a determined IPv6 attacker (see D5) | Distributed registration from many IPs / IPv6 /64 rotation |
...
7. **CSP `'wasm-unsafe-eval'` allows WebAssembly compilation in strict mode**: The production CSP retains `'wasm-unsafe-eval'` because `hash-wasm` (the WASM Argon2id implementation used by the vault KDF in `src/lib/crypto/crypto-client.ts`) cannot run without it. An attacker who successfully lands an XSS that survives `'strict-dynamic'` could then compile a WebAssembly module, partially bypassing the `'unsafe-eval'` defense. Mitigations: (a) `'strict-dynamic'` still gates the initial script execution, so an XSS must clear that bar first; (b) `'unsafe-eval'` (JS eval/Function) is NOT permitted in strict mode; (c) `worker-src 'self'` constrains WASM-in-Worker loads to same-origin only. Removing the directive would break vault setup / unlock entirely. Eliminating this risk would require migrating to a non-WASM Argon2id implementation (e.g. pure-JS `@noble/hashes/argon2id`, accepting the speed loss). Supply-chain residual risk: hash-wasm ships a prebuilt WASM tarball from a single maintainer; mitigated by exact-pin in `package.json` + npm lockfile integrity. Tracked as audit finding M2 (2026-05; library swapped from argon2-browser → hash-wasm in 2026-05 A06-2 — RFC 9106 conformance verified by cross-impl agreement with @noble/hashes in `argon2-vectors.test.ts`).

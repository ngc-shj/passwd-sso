...
| D5a: Bearer-token caller bypasses tenant IP restriction | `/api/v1/*`, `/api/extension/*`, `/api/mcp`, `/api/mobile/*`, SCIM | `enforceAccessRestriction` / `checkAccessRestrictionWithAudit` enforce tenant `allowedCidrs` on Bearer-authenticated routes (incl. the MCP gateway POST + SSE and iOS token lifecycle) as well as session-cookie routes | Restriction only applies when `allowedCidrs` is configured; unconfigured tenants have no IP restriction on any route |

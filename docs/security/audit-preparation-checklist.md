...
- [ ] Operator (`op_*`) token model reviewed: per-operator tokens minted at `/admin/tenant/operator-tokens`; `ADMIN_API_TOKEN` env var is not used by the application itself
...
- [ ] Audit outbox durability confirmed: `audit_outbox` written in-tx with business logic; drained by `audit-outbox-worker` (entry point `scripts/audit-outbox-worker.ts`; implementation in `src/workers/audit-outbox-worker.ts`) with exponential backoff; permanently failed rows dead-lettered as `AUDIT_OUTBOX_DEAD_LETTER`

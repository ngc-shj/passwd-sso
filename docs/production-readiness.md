...
| # | 優先度 | 項目 | 状態 | 備考 |
|---|--------|------|------|------|
| 1.1 | 必須 | CI/CD パイプライン構築 | 未着手 | GitHub Actions: lint → test → build → セキュリティスキャン → デプロイ。テスト 121 ファイル・20,086 行が既存のため、ワークフロー定義のみで即効果 |
...
| # | 優先度 | 項目 | 状態 | 備考 |
|---|--------|------|------|------|
| 2.1 | 必須 | 環境変数バリデーション | 未着手 | t3-env または Zod スキーマで起動時に一括検証。`ORG_MASTER_KEY` 未設定での起動を防止 |

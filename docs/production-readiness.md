...
| 5.2 | 強く推奨 | 依存パッケージのライセンス監査 | 一部対応 | CI に lockfile ベースの自動監査を追加（`npm run licenses:check` / `licenses:check:ext`）。禁止: AGPL/GPL は fail。LGPL/MPL/CC-BY/BlueOak 等は要レビューとして警告し、法務判断を継続 |
...
1. `5.2` 依存パッケージのライセンス監査の完成 (現状: 一部対応)
2. `4.4` セキュリティスキャン自動化
3. `2.5` 並行セッション管理
4. `1.6` エラートラッキング
5. `4.2` カバレッジ対象の拡大

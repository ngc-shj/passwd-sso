...
### Deferred rollout: `requireFreshPasskey` is implemented but not yet attached to existing sensitive routes
...
- Reason:
  ...
  - Attaching `requireFreshPasskey` immediately would create dead-end `SESSION_STEP_UP_REQUIRED` paths for callers that do not yet have an in-product passkey reauth retry UX.

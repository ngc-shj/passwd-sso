...
- Forbidden patterns:
  - pattern: `600_000` in src/ outside `crypto-params.ts` — reason: PBKDF2 iteration re-declaration. Named exclusions (semantically distinct values, do NOT fold in): `src/lib/validations/common.server.ts` `KDF_PBKDF2_ITERATIONS_MIN` (minimum guard, not the default); `src/lib/prisma.ts:79` `max: 600_000` (pool config, unrelated); `src/lib/crypto/crypto-client.ts:22` `VERIFIER_PBKDF2_ITERATIONS` (passphrase-verifier iteration count, exported via CRYPTO_CONSTANTS, mirrored by `cli/src/lib/crypto.ts:17` — same value today by coincidence of policy, separate knob by design)

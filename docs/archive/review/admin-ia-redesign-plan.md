...
│   └─ オペレータートークン  /admin/tenant/machine-identity/operator-tokens
...
5. Add `src/__tests__/admin-i18n-key-coverage.test.ts` (sentinel for new keys — forward direction).
   ...
   - **MANDATORY exclusions** (round-1 finding T2 — without these the test is vacuous because the test file itself contains the literal key strings):
     ```bash
     grep -rn --exclude-dir=__tests__ --exclude='*-i18n-*.test.ts' --exclude='*.test.tsx' "${key}" src/
     ```
     OR equivalently in JS: `glob('src/**/*.{ts,tsx}', { ignore: ['**/__tests__/**', '**/*.test.{ts,tsx}'] })` and read each.
...
9. **Authorization regression test (round-1 finding S3)** — add `e2e/tests/admin-authz.spec.ts`:
   - For every URL in step 8's list, navigate as an unauthenticated user → expect HTTP 404 (per `notFound()` in tenant/team layout).
   - For every URL in step 8's list, navigate as a NON-admin user (e.g. plain tenant member with no admin role) → expect HTTP 404.
...
15. Run `npx vitest run` — all unit/integration tests pass.
16. Run `npx next build` — production build succeeds.
17. Run `scripts/pre-pr.sh` — all CI gates pass locally.
...
**Reusable section components (round-1 F10/T8 decomposition)**:

| Path | Purpose |
|---|---|
| `src/components/team/members/add-from-tenant-section.tsx` | Search-and-add flow for existing tenant members |
| `src/components/team/members/invite-by-email-section.tsx` | Email + role + invite-link generation |
| `src/components/team/members/pending-invitations-list.tsx` | Pending invitations list with cancel + copy-link |
...
| `src/app/[locale]/admin/tenant/audit-logs/layout.tsx` | Remove sub-tab navItems (audit-logs is now leaf) |
| `src/app/[locale]/admin/teams/[teamId]/members/layout.tsx` | Remove sub-tab navItems |
| `src/components/settings/account/section-nav.test.tsx:82,93,104,115,126` | 5 fixture URLs that hard-code `/admin/tenant/security/*` paths — update to current valid admin URLs (or to a non-admin fixture like `/dashboard/settings/...` if the test only exercises link routing logic) |
...
After all `git mv` operations land their content elsewhere, the following directories become empty (or contain only orphaned `layout.tsx` / redirect-only `page.tsx` files) and must be removed:
...
- New assertions: `aria-current="page"` is set on active items in both desktop and mobile sheet variants (round-1 F7/T6).

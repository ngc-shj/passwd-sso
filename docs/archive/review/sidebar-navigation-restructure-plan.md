...
- F2: Tags section is an independent collapsible section with `[+]` button and tree structure
...
- F10: Existing folder/tag CRUD operations (create, edit, delete dialogs) work identically
...
1. **`sidebar-sections.tsx`**: Split `ManageSection` into `FoldersSection` + `TagsSection`. Remove Audit Log from `VaultManagementSection`. Each new section has its own `[+]` button (direct button, NOT dropdown) calling the respective `onCreate` handler.
...
8. Update `SidebarContentProps` interface: remove `activeAuditTeamId`, add `isPersonalAuditLog`. Update `use-sidebar-view-model.ts` params and return to match. Remove `activeAuditTeamId` from `VaultManagementSection` call site in `sidebar-content.tsx`
...
|------|-----------------|
| `sidebar-sections.test.tsx` | Replace ManageSection tests with FoldersSection + TagsSection tests; remove Audit Log from VaultManagementSection tests; add [+] button tests for each section |
...
- Personal vault: Folders and Tags render as separate collapsible sections with independent [+] buttons
...
- Team vault (Viewer): Folders and Tags sections visible (read-only, no [+] buttons); Security section NOT rendered; Settings section NOT rendered
...
- Folder/Tag CRUD: create, edit, delete dialogs still triggered correctly from new sections

...
- `getHintString` / `getAutocomplete`: identical logic; move once. Keep the `getHintString` associated-label comment from the cc version (the only textual diff) so no information is lost. (Implementation note / simplify pass: `getHintString` is exported — the detectors call it in `findKanaField`/`findPlainNameField`/`findConfNumCvvInForm` — but `getAutocomplete` is only used inside `findFieldByAutocomplete`, so it stays **module-private**, not exported. Net public surface: 4 exports + 1 private, not 5 exports.)

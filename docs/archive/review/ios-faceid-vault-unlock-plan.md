...
- UI: when `biometricUnlock != nil`, show a prominent "Unlock with \(biometryLabel)" button above the passphrase field. **NO auto-prompt** — the user must explicitly tap the button. (Device-testing revision: auto-invoking on `.onAppear` re-unlocked instantly the moment the user tapped Lock while looking at the device — the lock screen never appeared. Locking MUST show the lock screen and stay locked until an explicit action.) The passphrase field + Unlock button are ALWAYS present as fallback.
...
- Acceptance (device-manual): after Lock, the lock screen appears and STAYS (no instant re-unlock); tapping "Unlock with Face ID" → Face ID → unlocked; cancel → lock screen remains, passphrase usable.

...
- **Consumer-flow walkthrough** (UnlockResult is consumed by RootView.handleVaultUnlocked): `handleVaultUnlocked` reads `unlockResult.vaultKey` and `unlockResult.userId` to build the sync service, register QuickType identities, and populate the list — both present in the biometric-path UnlockResult (userId from the cache header, guarded non-empty). Identical shape to the passphrase path. ✓
...
- UI: when `biometricUnlock != nil`, show a prominent "Unlock with \(biometryLabel)" button above the passphrase field, and **auto-invoke it once on `.onAppear`** (the expected UX: the biometric sheet pops immediately; if the user cancels, the button + passphrase remain). A one-shot `@State hasAutoPrompted` guards against re-prompting on every re-render. The passphrase field + Unlock button are ALWAYS present as fallback.
- Errors: biometric cancel/fail does NOT show a scary error — it silently leaves the passphrase fallback (optionally a subtle "Face ID canceled" caption). A `.cacheUnreadable`/`.biometricUnavailable` surfaces as "Use your passphrase to unlock."
- Acceptance (device-manual; the view's logic — auto-prompt-once guard, fallback presence — is the unit-relevant part but views are manual): on the lock screen, Face ID prompts automatically; success → unlocked; cancel → passphrase field usable; the button re-triggers biometric.

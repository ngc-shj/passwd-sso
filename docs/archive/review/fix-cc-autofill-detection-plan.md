...
- `conf.?num` (matches `conf_number`) — **REMOVED from the page-wide `CC_DETECT_RE.cvv` alternation (Phase 3, user-directed).** SC4 had accepted it as "same-page misfill within the trust boundary", but the concrete risk is sharper: when the real CVV field lacks `autocomplete="cc-csc"` (typical on JP sites), CVV detection falls to the regex fallback, which is first-match-wins over the whole document — an unrelated `conf_number` field (order/booking confirmation) appearing before the real CVV would receive the CVV write, potentially submitting the secret to a different form/log. Fix (two rounds): `conf.?num` is now a **co-located + CVV-signalled** fallback only — the exported `CC_CONF_NUM_RE` matches a `conf_number` field ONLY when ALL of:
  1. It is co-located with the card-number field: same `<form>`, OR (form-less pages) same `<table>`. A shared page wrapper (`#app`/`main`) is deliberately NOT co-location — an SPA wraps the whole page in one, which would re-admit an unrelated confirmation section (user finding, round 2 of this sub-fix). `isCoLocatedWith`.
  2. It carries a CVV-specific signal — `type="password"` OR `maxLength` 3–4 (`looksLikeCvvField`) — so a generic plain-text confirmation-number field, even inside the same form, is rejected.
  3. All strong page-wide CVV signals (cvv/cvc/csc/security_code/`\bcard.?verif`) missed first — `findConfNumCvvInForm` runs only when `!cvv`.

  Both copies (lib + .js) mirror `looksLikeCvvField` / `isCoLocatedWith` / `findConfNumCvvInForm`; parity test pins `CC_CONF_NUM_RE` and asserts `conf` is absent from `CC_DETECT_RE.cvv`. Security regression tests: separate-section conf_number → cvv null (detection AND fill), `#app`/`main` wrapper → null, different-`<table>` → null, same-form-no-signal → null, same-form/same-table with signal → claimed, strong-signal-wins. Mutation-verified: reverting the `<table>` scope to a loose common-ancestor reddens the `#app`-wrapper test; dropping the CVV-signal requirement reddens the no-signal test. Residual (accepted): a same-container 3–4-char non-CVV confirmation field could still match — acceptable defense-in-depth given the name+placement+attribute combination.

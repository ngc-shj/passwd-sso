...
- `conf.?num` (ドスパラ `conf_number`) is a genericism risk ("confirmation number") — **SC4 verdict from security review: ACCEPTED.** CVV assignment only happens inside a form where a card-number field was already detected; residual risk is same-page field misplacement within the trust boundary the user accepted by picking the entry. Counter-fixture required (C7 negative row).

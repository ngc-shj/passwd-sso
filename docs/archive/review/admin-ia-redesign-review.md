...
- T13a (pre-pr.sh manual-test enforcement): borderline scope; flagged for user decision in round 1; not applied. **Anti-Deferral check**: out of scope (different feature) — manual-test artifact creation itself is in scope (Phase 2 deliverable per R35); programmatic enforcement of its presence is a separate CI-tooling concern. TODO marker not added because the enforcement gap is documented in this review and traceable.

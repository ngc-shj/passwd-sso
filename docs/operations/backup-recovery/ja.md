...
バックアップが動いていないことを確認したうえで削除してください。エラーメッセージが
正確なパスを表示します。
...
これを行うフラグは意図的に用意していません。スクリプト内で奪い取る実装は、試した
どの方式もレースしました — 2 つの実行がそれぞれ「古い」と判定でき、後に動いた方が
先行の作り直したロックを消してしまいます。ディレクトリさえ消えていれば、あとは
`mkdir` だけで、これは原子的です。
...
- **失敗した実行の中身は残しますが、無期限ではありません。** 検証に失敗した実行は
  診断用に `<stamp>.FAILED` として保持します。過去ぶんは `BACKUP_RETAIN` と
  `BACKUP_FAILED_MAX_AGE_DAYS`（既定 7 日）の両方で上限を設けていますが、削除は
  ダンプの前に走るため、失敗した実行のぶんは次回実行までその上限に上乗せされます。保持された
...
- **アーカイブは平文の機密です。** 全ての暗号文ブロブ、監査ログ、ラップ済み鍵を
  ...
  - `df(1)` と `mount(8)` が答えられない場合も拒否します。**macOS は mount を
    ...
    Linux では `/proc/self/mountinfo` を読むため、どちらのコマンドも不要です。
  - macOS には同等の表がないため、`df` がマウント点を名指しし、`mount(8)` に
    その行が**ちょうど 1 本**あることを要求します。同じマウント点を主張する行が
    2 本ある場合（FUSE のソース名は利用者が自由に綴れるため、ローカル利用者なら
    誰でも作れます）、判定は「誤り」ではなく「不定」となり、そのマウントを外すか
    下記のフラグを設定するまで実行は停止します。
...
- **`BACKUP_DATABASES` は既定値であって探索結果ではありません。** マネージド
  インスタンスでは名前が Compose 構成と異なるのが普通です。スクリプトはテンプレート
  以外のすべてのデータベースを列挙し、一覧に無いものを警告して MANIFEST の
  `not_backed_up:` に記録します。
  - 接続が禁止されている（`datallowconn=false`）データベースも列挙し、
    `名前[no-connect]` として記録します。接続禁止は保守中の通常状態であって、
    データが無いことを意味しません。

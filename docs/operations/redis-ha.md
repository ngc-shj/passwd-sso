...
1. **Session cache with tombstone-based revocation propagation** — the proxy auth gate caches session validity in Redis with a 30 s positive TTL and 5 s negative TTL (`SESSION_CACHE_TTL_MS` / `NEGATIVE_CACHE_TTL_MS`, hardcoded in `src/lib/validations/common.server.ts`; not configurable via environment). When a session is revoked, a 30 s tombstone (`TOMBSTONE_TTL_MS`) is written to Redis so that it cannot expire before a stale positive cache entry. Without Redis, tombstones do not propagate across processes.
